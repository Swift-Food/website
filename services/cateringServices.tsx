...
class CateringService {
  async searchMenuItems(query: string, filters?: SearchFilters): Promise<SearchResponse> {
  ...
  async submitCateringOrder(
    ...
    const estimatedTotal = orderItems.reduce((sum, order) => sum + order.totalPrice, 0);
    ...
    const createDto: CreateCateringOrderDto = {
      ...
      deliveryAddress: `${contactInfo.addressLine1}${contactInfo.addressLine2 ? ', ' + contactInfo.addressLine2 : ''}, ${contactInfo.city}, ${contactInfo.zipcode}`,
  ...
  async createAddress(userId: string, contactInfo: ContactInfo): Promise<string> {
    ...
    console.log('Creating address for user:', userId, JSON.stringify(createAddressDto));
